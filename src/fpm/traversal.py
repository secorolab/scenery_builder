...
def traverse_nodes_with_parent_sparql(open_set_ds, graph, root, neighbour_selector):
    ...
    while not open_set.is_empty():
        ...
        if node not in visited:
            ...
            neigh = list(
                graph.query(neighbours, initBindings={"root": root, "node": node})
            )
...
def traverse_nodes_with_parent_and_edges(
    open_set_ds, graph, root, neighbour_filter=None
):
    ...
    while not open_set.is_empty():
        ...
        if (node, parent) not in visited:
            ...
            visited[(parent, node)] = (
                True  # TODO: do want to have this? configuration option?
            )
